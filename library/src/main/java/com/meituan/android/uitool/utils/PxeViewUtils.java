...
public class PxeViewUtils {
    ...
    @Nullable
    public static List<PxeViewInfo> getTargetActivityViews(Activity targetActivity) {
        if (targetActivity == null || targetActivity.isFinishing()) {
            return null;
        }
        try {
            List<PxeViewInfo> viewsInfo = new ArrayList<>();
     ...
     * @param viewInfo 集合
     */
    public static void traverseRecordViews(View view, List<? super PxeViewInfo> viewInfo) {
        if (view == null) {
            return;
        }
        Object ignoreTag;
        //如果view设置了ignore标签,则过滤它和它的子view
        if ((ignoreTag = view.getTag(R.id.pxe_ignore)) != null && ((Boolean) ignoreTag)) {
            return;
        }
        ...
        viewInfo.add(new PxeViewInfo(view));
    ...
    private static boolean isChildVisible(ViewGroup group) {
        ...
        for (int i = 0; i < count; i++) {
            ...
            if (v.getVisibility() == View.VISIBLE && PxeDimensionUtils.isRectInScreen(r)) {
                return true;
            }
        }
        return false;
    ...
    /**
     * 判断目标{@link PxeViewInfo} 中的位置,并向上迭代其父类, 判断可见性
     *
     * @param target 目标 viewInfo
     * @return 不可见返回true, 可见返回true
     */
    public static boolean isViewInfoNotVisible(PxeViewInfo target) {
        if (target == null) {
            return true;
        }
        if (target.getRect().left >= PxeDimensionUtils.getScreenWidth()
                || target.getRect().top >= PxeDimensionUtils.getScreenHeight()) {
            return true;
        } else {//屏幕之内
    ...
    public static Object getBackground(View view) {
        ...
        if (drawable instanceof ColorDrawable) {
            return PxeResourceUtils.intToHexColor(((ColorDrawable) drawable).getColor());
            ...
            try {
                ...
                if (shader instanceof LinearGradient) {
                    ...
                    for (int i = 0, N = mColors.length; i < N; i++) {
                        sb.append(PxeResourceUtils.intToHexColor(mColors[i]));
                        if (i < N - 1) {
                            sb.append(" -> ");
                        }
                    }
